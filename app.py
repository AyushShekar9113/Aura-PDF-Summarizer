...
# # -------------------------------
# st.set_page_config(page_title="Aura PDF 🤖", layout="centered", page_icon="🤖")
# st.markdown(
#     """
#     <style>
#     .stApp {
...
# st.title("🌟 Aura - Ask Your PDF in Your Language")
# st.markdown("Upload a PDF and ask questions in your preferred language. Aura will answer clearly and politely!")
...
#     "Kannada": "kn",
# }
# selected_lang = st.selectbox("🌍 Translate Answer To", list(lang_map.keys()))
# dest_lang = lang_map[selected_lang]
...
#     with st.spinner("🤖 Aura is reading and thinking..."):
...
#         if not docs:
#             st.error("❌ Could not extract text from PDF. Please upload a valid file.")
#             st.stop()

#         st.write(f"✅ Loaded {len(docs)} text chunks from the PDF")
...
#         # -------------------------------
#         aura_template = PromptTemplate.from_template(
#             """You are Aura 🌟, a kind, helpful assistant who explains PDF content clearly and conversationally.
...
# Aura's Friendly Answer:"""
#         )

#         parser = StrOutputParser()
#         chain = aura_template | llm | parser

#         # Combine last 3 interactions
#         past_context = "\n".join(
#             f"Q: {q}\nA: {a}" for q, a in st.session_state.history[-3:]
#         )
...
#         # Translate result
#         translated_result = GoogleTranslator(source='auto', target=dest_lang).translate(result)
...
#         # Display Answers
#         # -------------------------------
#         with st.expander("✨ Aura's Answer (English)"):
#             st.write(result)

#         with st.expander(f"🌐 Translated to {selected_lang}"):
...
#             for i, page in enumerate(pages[:3]):  # show first 3 pages only
...
#     "Is there a section about Python basics?"
# ]
